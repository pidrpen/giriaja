...
function exportMatrixKit({ product, orderDate, fioGi, pagePhrases }) {
  ...
  const items = [
    `1. Отчет «Статистика выгрузки и сравнение контрольных сумм спецификаций» ${ph('stat')} формата А4`,
    `2. Сводная ведомость нормативной трудоемкости операций на изделие ${pp} (Форма 1) ${ph('f1')} формата А4`,
    `3. Сводная ведомость нормативной трудоемкости операций по подразделениям на изделие ${pp} (Форма 2) ${ph('f2')} формата А4`,
    `4. Сводная ведомость нормативной трудоемкости работ по подразделениям и номенклатуре на изделие ${pp} (Форма 3) ${ph('f3')} формата А4`,
    `5. Сводная ведомость плановой себестоимости продукции-материальных затрат на изделие ${pp} ${ph('mat')} формата А4`,
  ];
  items.forEach((item) => {
